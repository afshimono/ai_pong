...
class LocationDirectionModel:
    ...
    __ballYSpeeds = []  #array with edges for ball speed states
    __ballYSpeedNames = []   #array with ball speed state names
    __GAME_STATES = []    #list of all game states.
    __GAME_ACTIONS = ['UP','DOWN','STOP']     #list of possible actions for the player
    __REWARD_STATES = []        #list of all state names and actions to be rewarded 1
    __LAST_STATE = None
    __REWARD_CSV_PATH = ''
    __POLICY_CSV_PATH = '' 
    ...
    def __init__(self, WINDOWWIDTH, WINDOWHEIGHT, LINETHICKNESS, PADDLEOFFSET,PADDLESIZE,SCOREWINNER):
        ...
        self.__BALL_Y_SPEED_STATES = 7   
        self.__PLAYER_PADDLE_STATES = 18   
        ...
        for player_paddle_area_id in range(0,self.__PLAYER_PADDLE_STATES):
            self.__playerPaddleCoordinates.append([0,PADDLEOFFSET + LINETHICKNESS,(24 + player_paddle_area_id * 24),24+ (player_paddle_area_id + 1)* 24 ])
            self.__playerPaddleNames.append('P1_'+str(player_paddle_area_id))
        #print(str(playerPaddleNames))  #this is a debug print

        self.__ballYSpeeds = [-6,-4,-2,0,2,4,6]
        for i in range(0,self.__BALL_Y_SPEED_STATES):           
            self.__ballYSpeedNames.append('SPD_'+str(self.__ballYSpeeds[i]))
        #print(str(self.__ballYSpeedNames))  #this is a debug print

        for ball_state in self.__ballStateNames:
            for player_state in self.__playerPaddleNames:
                for ball_speed_state in self.__ballYSpeedNames:
                    self.__GAME_STATES.append(ball_state+'_'+player_state+'_'+ball_speed_state)
        #print(str(GAME_STATES))  #this is a debug print

        #sets REWARD_STATES
        self.__getRewardStatesActions()



        #sets csv locations
        self.__POLICY_CSV_PATH = 'q_learning_policy_' + self.getName() + str(self.getPLAYER_PADDLE_STATES()) + '_' + str(self.getBALL_Y_SPEED_STATES()) + '.csv'
        self.__REWARD_CSV_PATH = 'q_learning_reward_' + self.getName() + str(self.getPLAYER_PADDLE_STATES()) + '_' + str(self.getBALL_Y_SPEED_STATES()) + '.csv'
    ...
    #this function returns a list of lists. Each list contains a pair of state name and action. Those will be rewarded 1.
    def __getRewardStatesActions(self):
        #gets all states where the ball is below the paddle
        for i in range(len(self.__playerPaddleNames) - 1):     #skips the last state because it can't go below
            self.__REWARD_STATES.append(['B_'+str(i+5)+'_P1_'+str(i)+'_SPD_-4','DOWN'])
            self.__REWARD_STATES.append(['B_'+str(i+5)+'_P1_'+str(i)+'_SPD_-2','DOWN'])
            self.__REWARD_STATES.append(['B_'+str(i+5)+'_P1_'+str(i)+'_SPD_0','DOWN'])
            self.__REWARD_STATES.append(['B_'+str(i+5)+'_P1_'+str(i)+'_SPD_2','DOWN'])
            self.__REWARD_STATES.append(['B_'+str(i+5)+'_P1_'+str(i)+'_SPD_4','DOWN'])
            self.__REWARD_STATES.append(['B_'+str(i+5)+'_P1_'+str(i)+'_SPD_6','DOWN'])
        #gets all states where the ball is in the lower edge of the paddle, going down
        for i in range(len(self.__playerPaddleNames)-1):    
            self.__REWARD_STATES.append(['B_'+str(i+4)+'_P1_'+str(i)+'_SPD_2','DOWN'])
            self.__REWARD_STATES.append(['B_'+str(i+4)+'_P1_'+str(i)+'_SPD_4','DOWN'])
            self.__REWARD_STATES.append(['B_'+str(i+4)+'_P1_'+str(i)+'_SPD_6','DOWN'])
        #gets all steates where the ball is above the paddle
        for i in range(len(self.__playerPaddleNames) - 1):     #skips the last state because it can't go above
            self.__REWARD_STATES.append(['B_'+str(i+2)+'_P1_'+str(i+1)+'_SPD_4','UP'])
            self.__REWARD_STATES.append(['B_'+str(i+2)+'_P1_'+str(i+1)+'_SPD_2','UP'])
            self.__REWARD_STATES.append(['B_'+str(i+2)+'_P1_'+str(i+1)+'_SPD_0','UP'])
            self.__REWARD_STATES.append(['B_'+str(i+2)+'_P1_'+str(i+1)+'_SPD_-2','UP'])
            self.__REWARD_STATES.append(['B_'+str(i+2)+'_P1_'+str(i+1)+'_SPD_-4','UP'])
            self.__REWARD_STATES.append(['B_'+str(i+2)+'_P1_'+str(i+1)+'_SPD_-6','UP'])
        #gets all the states where the ball is on the upper edge of the paddle, going up
        for i in range(1,len(self.__playerPaddleNames)):    
            self.__REWARD_STATES.append(['B_'+str(i+3)+'_P1_'+str(i+1)+'_SPD_-2','UP'])
            self.__REWARD_STATES.append(['B_'+str(i+3)+'_P1_'+str(i+1)+'_SPD_-4','UP'])
            self.__REWARD_STATES.append(['B_'+str(i+3)+'_P1_'+str(i+1)+'_SPD_-6','UP'])
        #if the ball is right in the middle of the paddle, stops.
        for i in range(len(self.__playerPaddleNames)):
            for ball_speed in self.__ballYSpeeds:
                self.__REWARD_STATES.append(['B_'+str(i+2)+'_P1_'+str(i)+'_SPD_'+str(ball_speed),'STOP'])
    ...
    #returns the ball speed state, given the number of possible states.
    def getBallSpeedState(self,ballXspeed,ballYspeed):
        #print(str(ballSpeedCoordinates))    #this is a debugging print
        if ballYspeed == 0:
            return 'SPD_0'
        elif -2.5 <= ballYspeed <= 1:
            return 'SPD_-2'
        elif -4.5 <= ballYspeed <= -3:
            return 'SPD_-4'
        elif -6.5 <= ballYspeed <= -5:
            return 'SPD_-6'
        elif 5 <= ballYspeed <= 6.5:
            return 'SPD_6'
        elif 3 <= ballYspeed <= 4.5:
            return 'SPD_4'
        elif 1 <= ballYspeed <= 2.5:
            return 'SPD_2'                
        print('error out of bounds in getBallSpeedState!')
        return 'error out of bounds!'

    def getGameState(self,ballX,ballY,paddle1,paddle2,ballXspeed,ballYspeed):
        return self.getBallState(ballX,ballY) + '_' + self.getPlayerPaddleState(paddle1) + '_' + self.getBallSpeedState(ballXspeed,ballYspeed)

    def getName(self):
        return 'location_direction_model'
    ...
    def getNewRewardMatrix(self):
        ...
        for reward_state in self.__REWARD_STATES:
            reward_dataframe.set_value(reward_state[0],reward_state[1],1)
    ...
    ### GETTERS AND SETTERS
    def getBALL_Y_SPEED_STATES(self):
        return self.__BALL_Y_SPEED_STATES
